...
game.import('mode',function(lib,game,ui,get,ai,_status){
	return {
				...
				var createCardDialog=function(){
					...
					for(var i=0;i<cardDialog.buttons.length;i++){
						if(cardDialog.buttons[i].node.info.innerHTML.indexOf('随从')!=-1){
							...
							cardDialog.buttons[i].node.info.innerHTML = buttonName[5][1] + '牌' + buttonName[2] + '血';
						}
		...
		element:{
		    content: {
		        gainattack: function () {
		            if (!player.node.attack) return;
		            if (num > 0) {
		                player.changeattack(num, false);
		                game.log(player, '增加了' + get.cnNumber(num) + '点攻击');
		            }
		        },
		        loseattack: function () {
		            if (!player.node.attack) return;
		            if (player.attack <= 0) return;
		            if (num > player.attack) num = player.attack;
		            game.log(player, '减少了' + get.cnNumber(num) + '点攻击')
		            player.changelili(-num);
		        },
		        gainmaxattack: function () {
		            if (!player.node.attack) return;
		            if (num > 0) {
		                game.log(player, '增加了' + get.cnNumber(num) + '点攻击上限');
		            }
		            if (num < 0) {
		                if (-num > player.maxattack) {
		                    num = -player.maxattack;
		                }
		                game.log(player, '减少了' + get.cnNumber(num) + '点攻击上限');
		            }
		            player.maxattack += num;
		            player.updateattack();
		        },
		        changeattack: function () {
		            event.trigger('changeattack');    
		            if (!player.node.attack) return;
		            player.attack += num;
			...
			player: {
			    gainattack: function () {
			        var next = game.createEvent('gainattack');
			        next.player = this;
			        var nocard, nosource;
			        ...
			        next.setContent('gainattack');
			        return next;
			    },
			    loseattack: function () {
			        var next = game.createEvent('loseattack');
			        next.player = this;
			        var nocard, nosource;
			        var event = _status.event;
			        for (var i = 0; i < arguments.length; i++) {
			        ...
			        if (next.num == undefined) next.num = 1;
			        if (next.num <= 0) _status.event.next.remove(next);
			        next.setContent('loseattack');
			        return next;
			    },
			    updateattack: function () {
			        var attack = this.node.attack;
			        attack.style.transition = 'none';
			        if (this.isMin()) {
			            if (this.attack == Infinity) {
			                attack.innerHTML = '攻∞';
			            }
			            else if (this.maxattack < 10 && _status.stonetrans != true) {
			                game.broadcastAll(function(player){
			                    var img = document.createElement('div');
			                    img.setBackgroundImage('extension/东方project/' + player.attack + '.jpg');
			                    img.style.backgroundSize = 'cover';
			                    img.style.width = '100%';
			                    img.style.height = '100%';
			                    img.style.transform = 'translateY(-200px)';
			                    attack.appendChild(img);
			                    ui.refresh(img);
			                    img.style.transform = '';
			                }, this);
			            }
			            else {
			                attack.innerHTML = '攻'+this.attack;
			                attack.classList.add('text');
			            }
			        } else {
			            if (parseInt(this.maxattack) == parseFloat(this.maxattack)) {
			                this.attack = this.maxattack;
			            } else {
			                this.attack = parseInt(this.maxattack)
			            }
			            if (this.attack == Infinity) {
			                attack.innerHTML = '\xa0\xa0攻击：∞';
			            }
			            else { 
			                attack.innerHTML = '\xa0\xa0攻击：' + this.attack;
			                attack.classList.add('text');
			            }
			        }
			        setTimeout(function () {
			            attack.style.transition = '';
			        });
			        return this;
			    },
			    gainmaxattack:function () {
			        var next = game.createEvent('gainmaxattack');
			        next.player = this;
			        next.num = 1;
			        for (var i = 0; i < arguments.length; i++) {
			        ...
			        next.setContent('gainmaxattack');
			        return next;
			    },
			    changeattack:function (num, popup) {
			        var next = game.createEvent('changeattack', false);
			        next.num = num;
			        if (popup != undefined) next.popup = popup;
			        next.player = this;
			        next.setContent('changeattack');
			        return next;
			    },
				init:function(player){
				    if (!player.isMin() || player.forcemin) {
				        ...
				        player.updateActCount();
				        if (!player.node.attack) {
				            player.node.attack = ui.create.div('.handcards', player);
				        }
				        if (typeof player.attack !== 'number') {
				            player.attack = 1;
				        }
				        if (typeof player.maxattack !== 'number') {
				            player.maxattack = 1;
				        }
				        player.updateattack();
				    } else {
				        ...
				        if (!player.node.attack) {
				            player.node.attack = ui.create.div('.identity', player);
				        }
				        if (typeof player.attack !== 'number') {
				            player.attack = info[6][0];
				        }
				        if (typeof player.maxattack !== 'number') {
				            player.maxattack = info[6][1];
				        }
				        player.updateattack();
				    }
				},
		...
		characterPack:{
			mode_stone:{
				stone_tutengyongshi:['male','wei',5,['shaman_jili'],['minskin','stone'],[4,2,'shaman'],[2,2]],
				stone_xuejuren: ['male', 'wei', 2, ['shaman_xueju'], ['minskin', 'stone'], [1, 1, 'shaman'], [1, 1]],
				stone_tuyuansu: ['male', 'qun', 7, ['chaofeng'], ['minskin', 'stone'], [5, 4, 'shaman'], [2, 2]],
				stone_huoyuansu: ['male', 'shu', 5, ['shaman_huoxi'], ['minskin', 'stone'], [4, 3, 'shaman'], [2, 2]],
				stone_fachao: ['male', 'wei', 5, ['shaman_tuteng', 'shaman_fachao'], ['minskin', 'stone'], [3, 0, 'shaman'], [1, 1]],
				stone_huoshe: ['male', 'shu', 5, ['shaman_tuteng', 'shaman_huoshe'], ['minskin', 'stone'], [3, 0, 'shaman'], [1, 1]],
				stone_huoli: ['male', 'wei', 3, ['shaman_tuteng', 'shaman_huoli'], ['minskin', 'stone'], [2, 0, 'shaman'], [1, 1]],
				stone_huoyanweishi: ['male', 'shu', 5, ['shaman_zhuhuo'], ['minskin', 'stone'], [4, 1, 'shaman'], [2, 2]],
				stone_tutengshi: ['female', 'wei', 4, ['shaman_peiyu'], ['minskin', 'stone'], [3, 3, 'shaman'], [2, 2]],
				stone_shachuisaman: ['male', 'qun', 4, ['shaman_fengnu'], ['minskin', 'stone'], [4, 4, 'shaman'], [3, 3]],
				stone_wanshiyuansu: ['male', 'qun', 3, ['shaman_zoushi'], ['minskin', 'stone'], [3, 1, 'shaman'], [2, 2]],
				stone_shalinxingzhe: ['male', 'qun', 5, ['shaman_anhun'], ['minskin', 'stone'], [4, 2, 'shaman'], [2, 2]],

				stone_kuangyedoushi: ['male', 'wu', 4, ['druid_nuhuo'], ['minskin', 'stone'], [4, 2, 'druid'], [2, 2]],
				stone_conglinshouwei: ['male', 'wu', 5, ['druid_huwei'], ['minskin', 'stone'], [4, 2, 'druid'], [2, 2]],
				stone_baohuzhishu: ['male', 'qun', 8, ['chaofeng'], ['minskin', 'stone'], [6, 4, 'druid'], [2, 2]],
				stone_liebao: ['male', 'wei', 3, ['stone_chongfeng'], ['minskin', 'stone'], [3, 2, 'druid'], [3, 3]],
				stone_zongxiong: ['male', 'shu', 6, ['chaofeng'], ['minskin', 'stone'], [4, 2, 'druid'], [1, 1]],
				stone_baoqishi: ['female', 'wei', 3, ['druid_chengzhang'], ['minskin', 'stone'], [2, 2, 'druid'], [1, 1]],
				stone_renyaqishi: ['female', 'wei', 2, ['druid_renya'], ['minskin', 'stone'], [1, 1, 'druid'], [1, 1]],
				stone_huangyeqishi: ['male', 'wei', 6, ['druid_chicheng'], ['minskin', 'stone'], [5, 2, 'druid'], [3, 3]],
				stone_huoshanxiemu: ['male', 'wei', 7, ['druid_juhuo', 'chaofeng'], ['minskin', 'stone'], [5, 6, 'druid'], [2, 2]],
				stone_conglinxiaoshou: ['male', 'wei', 5, ['druid_yuehuo'], ['minskin', 'stone'], [4, 4, 'druid'], [2, 2]],
				stone_lindishuyao: ['female', 'wei', 5, ['druid_yeyou'], ['minskin', 'stone'], [3, 3, 'druid'], [2, 2]],
				stone_xunmenglong: ['male', 'wei', 2, ['druid_qicheng'], ['minskin', 'stone'], [3, 3, 'druid'], [3, 3]],

				stone_caoyuanshi: ['male', 'qun', 6, ['hunter_nuhou'], ['minskin', 'stone'], [5, 2, 'hunter'], [2, 2]],
				stone_leiouke: ['male', 'shu', 4, ['hunter_zhanhuo'], ['minskin', 'stone'], [3, 1, 'hunter'], [2, 2]],
				stone_huofu: ['male', 'qun', 3, ['stone_chongfeng'], ['minskin', 'stone'], [3, 4, 'hunter'], [3, 3]],
				stone_misha: ['male', 'shu', 5, ['chaofeng'], ['minskin', 'stone'], [3, 3, 'hunter'], [1, 1]],
				stone_jiewangzhu: ['male', 'wu', 1, ['hunter_jiewang'], ['minskin', 'stone'], [1, 2, 'hunter'], [2, 2]],
				stone_xunshoushi: ['male', 'qun', 5, ['hunter_xunshou'], ['minskin', 'stone'], [4, 3, 'hunter'], [2, 2]],
				stone_senlinlang: ['male', 'qun', 2, ['hunter_qunxi'], ['minskin', 'stone'], [1, 2, 'hunter'], [1, 1]],
				stone_tujiu: ['male', 'qun', 4, ['hunter_mishi'], ['minskin', 'stone'], [3, 2, 'hunter'], [2, 2]],
				stone_muyangren: ['male', 'qun', 5, ['hunter_muyang'], ['minskin', 'stone'], [4, 3, 'hunter'], [2, 2]],
				stone_jujishou: ['male', 'qun', 3, ['hunter_juji'], ['minskin', 'stone'], [2, 2, 'hunter'], [1, 1]],
				stone_damoshatuo: ['male', 'qun', 4, ['hunter_dusha'], ['minskin', 'stone'], [3, 3, 'hunter'], [2, 2]],
				stone_huangjialeixiang: ['male', 'qun', 3, ['hunter_chuanlin'], ['minskin', 'stone'], [2, 3, 'hunter'], [1, 1]],

				stone_shuiyuansu: ['male', 'wei', 6, ['mage_bingdong'], ['minskin', 'stone'], [4, 2, 'mage'], [2, 2]],
				stone_wushixuetu: ['female', 'wu', 1, ['mage_zhufa'], ['minskin', 'stone'], [1, 2, 'mage'], [1, 1]],
				stone_huoyao: ['male', 'shu', 3, ['mage_lieyan'], ['minskin', 'stone'], [3, 1, 'mage'], [2, 2]],
				stone_falifulong: ['male', 'shu', 1, ['mage_tunfa'], ['minskin', 'stone'], [1, 1, 'mage'], [1, 1]],
				stone_yingxiongzhihun: ['male', 'wei', 1, ['mage_minghuo'], ['minskin', 'stone'], [1, 2, 'mage'], [1, 1]],
				stone_shifazhe: ['male', 'qun', 5, ['mage_shifa'], ['minskin', 'stone'], [3, 3, 'mage'], [2, 2]],
				stone_aoshushi: ['male', 'qun', 4, ['mage_aoshu'], ['minskin', 'stone'], [3, 2, 'mage'], [2, 2]],
				stone_faqishi: ['male', 'qun', 6, ['mage_jili'], ['minskin', 'stone'], [4, 2, 'mage'], [2, 2]],
				stone_fuhuokaijia: ['male', 'qun', 4, ['mage_gushou'], ['minskin', 'stone'], [3, 3, 'mage'], [2, 2]],
				stone_kaodalalong: ['male', 'qun', 7, ['mage_yufa'], ['minskin', 'stone'], [5, 4, 'mage'], [3, 3]],
				stone_yanshushi: ['male', 'qun', 6, ['mage_yanshu'], ['minskin', 'stone'], [4, 4, 'mage'], [2, 2]],
				stone_xulingwushi: ['male', 'qun', 4, ['mage_pingxu'], ['minskin', 'stone'], [3, 3, 'mage'], [2, 2]],

				stone_hudunren: ['male', 'qun', 3, ['paladin_hudun'], ['minskin', 'stone'], [2, 2, 'paladin'], [1, 1]],
				stone_junxuguan: ['male', 'qun', 5, ['paladin_buji'], ['minskin', 'stone'], [4, 1, 'paladin'], [2, 2]],
				stone_yurenqishi: ['male', 'qun', 5, ['paladin_zhaochao'], ['minskin', 'stone'], [4, 2, 'paladin'], [2, 2]],
				stone_chidunweishi: ['male', 'qun', 4, ['paladin_chidun'], ['minskin', 'stone'], [3, 2, 'paladin'], [2, 2]],
				stone_liewangshouwei: ['male', 'qun', 6, ['paladin_shouwei'], ['minskin', 'stone'], [5, 2, 'paladin'], [3, 3]],
				stone_longwangpeiou: ['female', 'qun', 6, ['paladin_zhaohuan'], ['minskin', 'stone'], [5, 4, 'paladin'], [3, 3]],
				stone_baoweizhe: ['male', 'qun', 3, ['paladin_baowei'], ['minskin', 'stone'], [2, 1, 'paladin'], [1, 1]],
				stone_guiqishi: ['male', 'qun', 6, ['paladin_tuxi'], ['minskin', 'stone'], [5, 4, 'paladin'], [3, 3]],
				stone_shenmiqishou: ['male', 'qun', 6, ['paladin_miying'], ['minskin', 'stone'], [5, 4, 'paladin'], [3, 3]],
				stone_shixiangweishi: ['female', 'qun', 4, ['paladin_huashi'], ['minskin', 'stone'], [3, 3, 'paladin'], [2, 2]],
				stone_xuefanzhanshi: ['male', 'qun', 4, ['paladin_jinghua'], ['minskin', 'stone'], [4, 4, 'paladin'], [2, 2]],
				stone_xunmashi: ['male', 'qun', 4, ['paladin_moma'], ['minskin', 'stone'], [3, 2, 'paladin'], [2, 2]],

				stone_lieyanxiaogui: ['male', 'qun', 2, ['warlock_nonghuo'], ['minskin', 'stone'], [1, 4, 'warlock'], [2, 2]],
				stone_xiaoguishouling: ['male', 'qun', 4, ['warlock_zhaogui'], ['minskin', 'stone'], [3, 1, 'warlock'], [2, 2]],
				stone_xiaogui: ['male', 'qun', 2, [], ['minskin', 'stone', 'stonehidden'], [1, 1], [1, 1]],
				stone_kongjuzhanma: ['male', 'qun', 2, ['warlock_yongsheng'], ['minskin', 'stone'], [3, 1, 'warlock'], [2, 2]],
				stone_morishouwei: ['male', 'qun', 5, ['stone_chongfeng', 'warlock_zaihuo'], ['minskin', 'stone'], [4, 4, 'warlock'], [3, 3]],
				stone_xukongxingzhe: ['male', 'qun', 2, ['chaofeng'], ['minskin', 'stone'], [1, 1, 'warlock'], [1, 1]],
				stone_diyuhuo: ['male', 'qun', 6, ['warlock_yuhuo'], ['minskin', 'stone'], [5, 4, 'warlock'], [3, 3]],
				stone_diyuhuox: ['male', 'qun', 3, [], ['minskin', 'stone', 'stonehidden'], [2, 2, 'warlock'], [1, 1]],
				stone_heishitanfan: ['male', 'qun', 3, ['warlock_anyu'], ['minskin', 'stone'], [2, 2, 'warlock'], [1, 1]],
				stone_zhaohuanzhe: ['male', 'qun', 5, ['warlock_zhaohuan'], ['minskin', 'stone'], [4, 2, 'warlock'], [2, 2]],
				stone_meimo: ['male', 'qun', 3, ['warlock_huanmeng'], ['minskin', 'stone'], [2, 3, 'warlock'], [1, 1]],
				stone_tongkunvwang: ['male', 'qun', 3, ['warlock_tongku'], ['minskin', 'stone'], [2, 1, 'warlock'], [1, 1]],
				stone_xukongkongmo: ['male', 'qun', 4, ['warlock_tunshi'], ['minskin', 'stone'], [3, 3, 'warlock'], [2, 2]],
				stone_fukongmoyan: ['male', 'qun', 6, ['warlock_shijie'], ['minskin', 'stone'], [5, 4, 'warlock'], [3, 3]],

				stone_zhihuiguan: ['female', 'qun', 4, ['warrior_tongling'], ['minskin', 'stone'], [3, 2, 'warrior'], [2, 2]],
				stone_kuangzhanshi: ['male', 'qun', 4, ['warrior_baoluan'], ['minskin', 'stone'], [3, 1, 'warrior'], [2, 2]],
				stone_zhujiashi: ['male', 'qun', 3, ['warrior_zhujia'], ['minskin', 'stone'], [2, 1, 'warrior'], [1, 1]],
				stone_jiangong: ['male', 'qun', 3, ['warrior_jiangong'], ['minskin', 'stone'], [2, 2, 'warrior'], [1, 1]],
				stone_chidunshinv: ['female', 'qun', 6, ['warrior_tidun'], ['minskin', 'stone'], [5, 4, 'warrior'], [3, 3]],
				stone_yuanhou: ['male', 'qun', 3, ['chaofeng'], ['minskin', 'stone'], [2, 3, 'warrior'], [1, 1]],
				stone_heiyaoyaoshou: ['male', 'qun', 6, ['warrior_heiyao'], ['minskin', 'stone'], [5, 4, 'warrior'], [3, 3]],
				stone_honglongyongshi: ['male', 'qun', 3, ['warrior_fenyong'], ['minskin', 'stone'], [2, 3, 'warrior'], [1, 1]],
				stone_peilianshi: ['male', 'qun', 3, ['chaofeng', 'warrior_peilian'], ['minskin', 'stone'], [2, 2, 'warrior'], [1, 1]],
				stone_jingyingweishi: ['male', 'qun', 4, ['stone_chongfeng'], ['minskin', 'stone'], [4, 3, 'warrior'], [3, 3]],
				stone_mengmaren: ['male', 'qun', 5, ['warrior_chuanci'], ['minskin', 'stone'], [4, 4, 'warrior'], [2, 2]],
				stone_zhifuzhe: ['male', 'qun', 4, ['warrior_zhifu'], ['minskin', 'stone'], [3, 1, 'warrior'], [2, 2]],

				stone_daomufeizei: ['male', 'qun', 5, ['rogue_xunbao'], ['minskin', 'stone'], [4, 3, 'rogue'], [2, 2]],
				stone_qiezei: ['male', 'qun', 3, ['rogue_touqie'], ['minskin', 'stone'], [2, 2, 'rogue'], [1, 1]],
				stone_heitieairen: ['male', 'qun', 5, ['rogue_qiancang'], ['minskin', 'stone'], [4, 3, 'rogue'], [2, 2]],
				stone_tegong: ['male', 'qun', 4, ['rogue_touxi'], ['minskin', 'stone'], [3, 3, 'rogue'], [2, 2]],
				stone_haidaotoumu: ['male', 'qun', 1, ['rogue_zhaomu'], ['minskin', 'stone'], [2, 2, 'rogue'], [1, 1]],
				stone_haidao: ['male', 'qun', 2, [], ['minskin', 'stone', 'stonehidden'], [1, 2, 'rogue'], [1, 1]],
				stone_cike: ['male', 'qun', 2, ['rogue_cisha', 'stone_qianxing'], ['minskin', 'stone'], [1, 1, 'rogue'], [1, 1]],
				stone_duyanhaidao: ['male', 'qun', 4, ['rogue_duxing'], ['minskin', 'stone'], [3, 4, 'rogue'], [2, 2]],
				stone_gangtiewushi: ['male', 'qun', 4, ['rogue_shoudao'], ['minskin', 'stone'], [3, 2, 'rogue'], [2, 2]],
				stone_lifaji: ['male', 'qun', 3, ['rogue_lifa'], ['minskin', 'stone'], [2, 2, 'rogue'], [1, 1]],
				stone_shihualong: ['male', 'qun', 4, ['rogue_fusheng'], ['minskin', 'stone'], [3, 2, 'rogue'], [2, 2]],
				stone_xiushuihaidao: ['male', 'qun', 2, ['rogue_jielue'], ['minskin', 'stone'], [1, 2, 'rogue'], [1, 1]],
				stone_zousishangfan: ['male', 'qun', 5, ['rogue_jiaoyi'], ['minskin', 'stone'], [4, 3, 'rogue'], [2, 2]],

				stone_beijunmushi: ['male', 'qun', 2, ['priest_shengliao'], ['minskin', 'stone'], [1, 1, 'priest'], [1, 1]],
				stone_guanliyuan: ['male', 'qun', 3, ['priest_faxian'], ['minskin', 'stone'], [2, 1, 'priest'], [1, 1]],
				stone_linghunjisi: ['female', 'qun', 5, ['priest_hunwu'], ['minskin', 'stone'], [4, 2, 'priest'], [2, 2]],
				stone_heianjiaotu: ['male', 'qun', 4, ['priest_zhufu'], ['minskin', 'stone'], [3, 2, 'priest'], [2, 2]],
				stone_guangyaozhizi: ['male', 'qun', 6, ['priest_guangyao'], ['minskin', 'stone'], [5, 3, 'priest'], [3, 3]],
				stone_longmianjiaoguan: ['male', 'qun', 3, ['priest_xundao'], ['minskin', 'stone'], [2, 2, 'priest'], [1, 1]],
				stone_shengdianzhishi: ['male', 'qun', 6, ['priest_puzhao'], ['minskin', 'stone'], [5, 4, 'priest'], [3, 3]],
				stone_suoxiaojishi: ['male', 'qun', 3, ['priest_suoxiao'], ['minskin', 'stone'], [2, 2, 'priest'], [1, 1]],
				stone_anyingzisi: ['male', 'qun', 5, ['priest_shixin'], ['minskin', 'stone'], [4, 4, 'priest'], [2, 2]],
				stone_guangmingquan: ['male', 'qun', 3, ['priest_shengshui'], ['minskin', 'stone'], [2, 0, 'priest'], [1, 1]],
				stone_muguangchulong: ['male', 'qun', 2, ['priest_muguang'], ['minskin', 'stone'], [1, 1, 'priest'], [1, 1]],
				stone_shenshengyongshi: ['male', 'qun', 5, ['priest_shengguang'], ['minskin', 'stone'], [4, 3, 'priest'], [2, 2]],

				stone_zhongshi: ['male', 'wei', 2, ['stone_zhongshi1'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_zhucangzhe: ['male', 'wei', 2, ['stone_zhucangzhe1'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_huoqiangshou: ['male', 'wei', 4, ['stone_huoqiangshou1'], ['minskin', 'stone'], [3, 1], [2, 2]],

				stone_lansaizhanshi: ['male', 'shu', 1, ['stone_chongfeng'], ['minskin', 'stone'], [1, 2], [2, 2]],
				stone_kutongsiseng: ['male', 'shu', 2, ['stone_kutongsiseng1'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_yuanguanying: ['male', 'shu', 4, ['stone_yuanguanying1'], ['minskin', 'stone'], [3, 1], [2, 2]],

				stone_dijieshicong: ['male', 'wu', 2, ['stone_dijieshicong1'], ['minskin', 'stone'], [1, 1], [1, 1]],
				stone_yaosaishouwei: ['male', 'wu', 2, ['stone_yaosaishouwei1'], ['minskin', 'stone'], [1, 1], [1, 1]],
				stone_famingjia: ['male', 'wu', 4, ['stone_famingjia1'], ['minskin', 'stone'], [3, 1], [2, 2]],

				stone_chilundashi: ['male', 'qun', 2, ['stone_chilundashi1'], ['minskin', 'stone'], [1, 1], [1, 1]],
				stone_hanguangzhizhe: ['male', 'qun', 3, ['stone_hanguangzhizhe1'], ['minskin', 'stone'], [2, 2], [1, 1]],
				stone_aihaozhihun: ['male', 'qun', 4, ['stone_aihaozhihun1'], ['minskin', 'stone'], [3, 1], [2, 2]],

				stone_fennuxiaoji: ['male', 'qun', 2, ['stone_fennuxiaoji1'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_juxingchanchu: ['male', 'qun', 2, ['stone_juxingchanchu1'], ['minskin', 'stone'], [2, 1], [2, 2]],
				stone_wuyi: ['male', 'qun', 3, ['jijiu'], ['minskin', 'stone'], [2, 2], [1, 1]],
				stone_langren: ['male', 'qun', 2, ['stone_qianxing'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_shishigui: ['male', 'qun', 3, ['stone_shishigui1'], ['minskin', 'stone'], [2, 1], [1, 1]],

				stone_fatiaozhuru: ['female', 'qun', 2, ['stone_fatiaozhuru1'], ['minskin', 'stone'], [1, 2], [1, 1]],
				stone_mingguangjisi: ['female', 'wu', 3, ['shushen'], ['minskin', 'stone'], [2, 1], [1, 1]],
				stone_nianqingjisi: ['female', 'wei', 3, ['stone_zhufu'], ['minskin', 'stone'], [2, 1], [1, 1]],
				stone_aomishouwei: ['female', 'qun', 3, ['biyue'], ['minskin', 'stone'], [2, 2], [1, 1]],
				stone_yanjingshe: ['female', 'qun', 4, ['stone_yanjingshe1'], ['minskin', 'stone'], [3, 2], [2, 2]],
				stone_zhiyuzhe: ['female', 'qun', 4, ['stone_zhiyu'], ['minskin', 'stone'], [3, 1], [2, 2]],
				stone_mafengzhuru: ['female', 'qun', 2, ['stone_mafengzhuru1'], ['minskin', 'stone'], [1, 2], [1, 1]],

				stone_shumiao: ['none', 'wu', 2, [], ['minskin', 'stone', 'stonehidden'], [1, 1], [1, 1]],
				stone_shuren: ['none', 'wu', 2, ['stone_chongfeng', 'stone_zibao'], ['minskin', 'stone', 'stonehidden'], [2, 2], [2, 2]],
				stone_shurenx: ['none', 'wu', 3, [], ['minskin', 'stone', 'stonehidden'], [2, 2], [1, 1]],
				stone_shurenxx: ['none', 'wu', 3, ['chaofeng'], ['minskin', 'stone', 'stonehidden'], [2, 2], [1, 1]],
				stone_youlinglang: ['none', 'qun', 3, ['chaofeng'], ['minskin', 'stone', 'stonehidden'], [2, 2], [1, 1]],
				stone_xiaojingling: ['none', 'qun', 2, ['xuying'], ['minskin', 'stone', 'stonehidden'], [1, 1], [1, 1]],
				stone_zhumo: ['none', 'qun', 3, [], ['minskin', 'stone', 'stonehidden'], [2, 2], [1, 1]],
				stone_jingxiang: ['none', 'qun', 3, ['stone_jingxiang', 'chaofeng'], ['minskin', 'stone', 'stonehidden'], [2, 0], [1, 1]],
				stone_shengguanghuwei: ['female', 'qun', 2, ['priest_shengguang'], ['minskin', 'stone', 'stonehidden'], [1, 1], [1, 1]],
				stone_liegou: ['none', 'qun', 1, ['stone_chongfeng'], ['minskin', 'stone', 'stonehidden'], [1, 2], [2, 2]],
				stone_mianyang: ['none', 'qun', 2, ['mage_mianyang'], ['minskin', 'stone', 'stonehidden'], [1, 0], [1, 1]],
				stone_qingwa: ['none', 'wu', 2, ['shaman_qingwa'], ['minskin', 'stone', 'stonehidden'], [1, 0], [1, 1]],
				stone_shengjiachong: ['none', 'qun', 2, ['chaofeng'], ['minskin', 'stone', 'stonehidden'], [1, 1], [1, 1]],

				stone_tuteng1: ['none', 'qun', 3, ['shaman_tuteng', 'chaofeng'], ['minskin', 'stone', 'stonehidden'], [2, 0], [1, 1]],
				stone_tuteng2: ['none', 'qun', 3, ['shaman_tuteng', 'shaman_zhuore'], ['minskin', 'stone', 'stonehidden'], [2, 0], [1, 1]],
				stone_tuteng3: ['none', 'qun', 3, ['shaman_tuteng', 'shaman_fali'], ['minskin', 'stone', 'stonehidden'], [2, 0], [1, 1]],
				stone_tuteng4: ['none', 'qun', 3, ['shaman_tuteng', 'shaman_zhiliao'], ['minskin', 'stone', 'stonehidden'], [2, 0], [1, 1]],
				stone_xinbing: ['none', 'qun', 3, [], ['minskin', 'stone', 'stonehidden'], [2, 0], [2, 2]],

				stone_siwangzhiyi: ['male', 'qun', 7, ['stone_mieshi'], ['minskin', 'stone', 'stonehidden', 'stonelegend'], [6, 4], [3, 3]],
				stone_alaikesita: ['female', 'qun', 7, ['stone_fushi'], ['minskin', 'stone', 'stonehidden', 'stonelegend'], [6, 4], [3, 3]],
				stone_yisela: ['female', 'qun', 7, ['stone_chenshui'], ['minskin', 'stone', 'stonehidden', 'stonelegend'], [6, 2], [3, 3]],
				stone_nuoziduomu: ['male', 'qun', 7, ['stone_shixu'], ['minskin', 'stone', 'stonehidden', 'stonelegend'], [6, 4], [3, 3]],
				stone_maligousi: ['male', 'qun', 7, ['stone_mowang'], ['minskin', 'stone', 'stonehidden', 'stonelegend'], [6, 2], [3, 3]],

				stone_aolajier: ['male', 'qun', 6, ['stone_chongfeng', 'shaman_fengnu', 'paladin_hudun', 'chaofeng'], ['minskin', 'stone', 'stonehidden', 'stonelegend_shaman'], [6, 4], [4, 4]],
				stone_andongni: ['male', 'qun', 7, ['stone_zhiyin'], ['minskin', 'stone', 'stonehidden', 'stonelegend_mage'], [6, 4], [3, 3]],
				stone_jialakesi: ['male', 'qun', 7, ['stone_bianshen'], ['minskin', 'stone', 'stonehidden', 'stonelegend_warlock'], [6, 0], [3, 3]],
				stone_jialakesix: ['male', 'qun', 6, ['stone_lianyu'], ['modeimage', 'stonehidden', 'stonespecial'],[], [1, 1]],
				stone_kelushi: ['male', 'qun', 6, ['stone_chongfeng'], ['minskin', 'stone', 'stonehidden', 'stonelegend_hunter'], [6, 5], [4, 4]],
				stone_geluomashi: ['male', 'qun', 6, ['stone_chongfeng', 'stone_jinu'], ['minskin', 'stone', 'stonehidden', 'stonelegend_warrior'], [6, 4], [4, 4]],
				stone_aidewen: ['male', 'qun', 7, ['stone_lianji'], ['minskin', 'stone', 'stonehidden', 'stonelegend_rogue'], [6, 3], [3, 3]],
				stone_sainaliusi: ['male', 'qun', 7, ['stone_shenyu'], ['minskin', 'stone', 'stonehidden', 'stonelegend_druid'], [6, 3], [3, 3]],
				stone_fuding: ['male', 'qun', 7, ['paladin_hudun', 'chaofeng', 'stone_fuchou'], ['minskin', 'stone', 'stonehidden', 'stonelegend_paladin'], [6, 3], [3, 3]],
				stone_weilun: ['male', 'qun', 7, ['stone_shenyou'], ['minskin', 'stone', 'stonehidden', 'stonelegend_priest'], [6, 6], [3, 3]],
			}
		},
		...
		game:{
			...
			initStone:function(){
				...
				for(i in lib.card){
					...
					} else if (lib.card[i].type == 'stonecard') {
					    lib.card[i].addinfo = '消耗：' + lib.card[i].stoneact;
					    lib.card[i].addinfomenu = '消耗：' + lib.card[i].stoneact + '费';
					}else if (typeof lib.card[i].stoneact === 'number'&& !lib.card[i].addinfo) {
						    lib.card[i].addinfo = lib.card[i].stoneact + '费' + lib.card[i].maxattack+'攻';
						    lib.card[i].addinfomenu = lib.card[i].stoneact + '费' + lib.card[i].maxattack + '攻';
						}
					}
		...
		skill:{
		    ...
		    _attackdamage:{
		        ...
		        filter: function (event, player) {
		            return player.isMin() && event.source && event.source.attack > 1;
		        },
		        popup: false,
		        forced: true,
		        unique: true,
		        ...
		        direct: true,
		        content: function () {
		            trigger.num += (trigger.source.attack - 1);
		        },
		    },
		    _attackrecover: {
		        trigger: {
		...
		translate:{
			...
			shaman_peiyu:'培育',
			shaman_peiyu_info:'你出场时，增加X点体力和体力上限以及攻击，X为友方图腾数',
			shaman_huoli:'活力',
